...
namespace CampusApp.Evaluation.Helper
{
    class Functions
    {
        ...
        public static List<DataModel.DataSource> convertQuestionDTOToDataSource(DTO.QuestionsDTO aQDTO, String aCourse, String aURL)
        {
            ...
            foreach (DTO.Choices item in aQDTO.multipleChoiceQuestions)
            {
                ...
                DataModel.DataSource ds = new DataModel.DataSource(item.text, lAnswer1, lAnswer2, lAnswer3, lAnswer4, lAnswer5, lAnswer6, aQDTO.evaluationUID, aQDTO.voteToken, aCourse, aURL);
                dsl.Add(ds);
            }

            foreach (string value in aQDTO.textQuestions)
            {
                DataModel.DataSource ds = new DataModel.DataSource(value, aQDTO.evaluationUID, aQDTO.voteToken, aCourse, aURL);
                dsl.Add(ds);
            }
        ...
        public static DTO.AnswersDTO convertDataSourcetoAnswersDTO(List<DataModel.DataSource> aDSL)
        {
            ...
            if (aDSL.Count > 0)
            {
                lDTO.voteToken = aDSL[0].VoteToken;
                lDTO.innerSection = aDSL[0].Course;
